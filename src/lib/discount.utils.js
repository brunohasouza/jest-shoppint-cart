...
export const calculateDiscount = (amount, quantity, condition) => {
    ...
    const [higherDiscount] = list.map((c) => {
        if (c.percentage) {
            return calculatePercentageDiscount(amount, { condition: c, quantity }).getAmount()
        } else if (c.quantity) {
            return calculateQuantityDiscount(amount, { condition: c, quantity }).getAmount()
        }
    }).sort((a, b) => b - a)
